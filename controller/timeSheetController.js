...
const timeSheetController = {
    ...
    fakecheckeIn: async (req, res) => {
        const year = 2024;
        const month = 4; // Tháng bắt đầu từ 0, nên không cần trừ 1
        const startDay = 1;
        const endDay = 30; // Last day of the month

        for (let day = startDay; day <= endDay; day++) {
            const chekIn = new Date(year, month - 1, day, 7, 30, 0);
            const checkOut = new Date(year, month - 1, day, 17, 30, 0); // month is zero-indexed
            ...
            var overTime = 0;
            if (checkOut.getTime() > timeOut.getTime()) {
                overTime = Math.abs(checkOut.getTime() - timeOut.getTime());
            ...
            try {
                let aTimesheet = await timeSheet.findOne({
                    employee: "NVT01",
                    year: year,
                    month: month,
                    date: day
                ...
                if (aTimesheet === null) {
                    ...
                    await aTimesheet.save();
                }
            } catch (error) {
                console.error(`Error processing day ${day}:`, error);
                return res.status(500).json({ message: `Error processing day ${day}`, error: error.message });
            }
        }

        return res.status(200).json("Timesheet entries processed successfully.");
